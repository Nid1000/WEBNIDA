...
let FacturacionService = class FacturacionService {
    ...
    decolectaBaseUrl = process.env.DECOLECTA_BASE_URL || 'https://api.decolecta.com/v1';
    ensureFolder() {
    ...
    async fetchReniecDni(dni, token) {
        if (!token)
            return null;
    ...
    async nextNumeroComprobante(tipo, serie) {
    ...
    async consultaReniecDni(numero, decolectaToken) {
    ...
    async consultaSunatRuc(numero, decolectaToken) {
    ...
    async emitir(usuarioId, params, decolectaToken) {
        ...
        const comprobanteExistente = pedido.comprobantes.find((c) => c.tipo === params.comprobante_tipo);
        ...
        if (params.comprobante_tipo === 'factura' &&
        ...
        const correlativo = await this.nextNumeroComprobante(params.comprobante_tipo, serie);
        ...
        try {
            ...
            if (params.tipo_documento === 'DNI') {
                ...
                doc.text(`Cliente: ${[identidad?.first_name, identidad?.first_last_name, identidad?.second_last_name].filter(Boolean).join(' ') || `${pedido.usuario?.nombre ?? ''} ${pedido.usuario?.apellido ?? ''}`.trim() || 'N/A'}`);
            ...
            doc
                .fontSize(10)
                .fillColor('gray')
                .text(`Esta es una representación impresa de la ${params.comprobante_tipo} electrónica generada por el sistema.`, { align: 'left' });
            doc.fillColor('black');
            doc.end();
...
` +
            ...
            const nombreCliente = params.tipo_documento === 'DNI'
            ...
            const direccionCliente = params.tipo_documento === 'RUC'
...
<svg width="1000" height="700" viewBox="0 0 1000 700" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="40" y="530" class="label">Fecha de emisión:</text><text x="260" y="530" class="val">${new Date().toLocaleString('es-PE')}</text>
            ...
            await (0, sharp_1.default)(Buffer.from(svg)).png().toFile(imgAbs);
        }
        catch {
        }
        ...
        const comprobante = {
            ...
            pedido_id: pedido.id,
            total,
            cliente: params.tipo_documento === 'DNI'
                ? {
                    nombre: [identidad?.first_name, identidad?.first_last_name, identidad?.second_last_name]
                        .filter(Boolean)
                        .join(' ') || `${pedido.usuario?.nombre ?? ''} ${pedido.usuario?.apellido ?? ''}`.trim() || 'Cliente',
                    dni: params.numero_documento,
                }
                : {
                    razon_social: identidad?.razon_social ?? identidad?.nombre_o_razon_social ?? 'Cliente',
    ...
    async misComprobantes(usuarioId) {
